...
<template>
  <UDashboardPanel id="members">
    <template #body>
      <UiPageContainer title="Members" size="max-w-5xl">
        <div class="space-y-4">
          <UiTable
          ...
          >
            <template #filters>
              ...
              />
              <UButton leading-icon="i-heroicons:plus" variant="soft" @click="addModalOpen = true">
                Add Member
              </UButton>
            </template>
          </UiTable>
        ...
        >
          <template #body>
            <div class="space-y-4">
              <p class="text-sm">
                Are you sure you want to remove <strong>{{ selectedMemberToDelete?.user.name }}</strong> ({{ selectedMemberToDelete?.user.email }}) from the organization? This action cannot be undone.
              </p>
              <div class="flex gap-2 justify-end">
                <UButton
                  variant="soft"
