...
<template>
  <div v-if="maintenance?.read_only" class="shrink-0 bg-default px-6 py-3 lg:px-8">
    <UAlert
      ...
      title="S2 - Registry maintenance in progress"
      :description="`New S2 - Registry writes are blocked while garbage collection is ${maintenance.phase}; in-flight writes may finish. S2 - Registry is read-only during collecting; pulls remain available.`"
