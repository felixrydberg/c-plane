    ...
    >
      ...
      <span class="pl-2.5 pr-2 py-1 text-xs text-muted flex items-center gap-1.5">
        <div
          ...
          :class="[data.environmentLabels.length > 0 ? 'size-2.5' : 'size-2', data.environmentLabels.length === 0 ? 'bg-muted' : '']"
          :style="data.environmentLabels.length > 0 ? { backgroundColor: data.color } : {}"
        />
        {{ data.label }}
      </span>
