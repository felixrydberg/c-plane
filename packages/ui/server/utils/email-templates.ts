...
export const createOrganizationInvitationEmailTemplate = ({
  organizationName,
  inviterName,
  url,
  action,
  ...
  html: createEmailLayout({
    ...
    message: inviterName
      ? `${inviterName} invited you to join ${organizationName}. Continue to ${action === 'signin' ? 'sign in' : 'create your account'} and accept the invitation.`
      : `You were invited to join ${organizationName}. Continue to ${action === 'signin' ? 'sign in' : 'create your account'} and accept the invitation.`,
    ctaLabel: action === 'signin' ? 'Sign in to accept invite' : 'Sign up to accept invite',
    ctaUrl: url,
    footer: 'This invitation expires in 30 days.',
  }),
});
