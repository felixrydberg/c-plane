...
if command -v deno.exe >/dev/null 2>&1 && deno.exe --version >/dev/null 2>&1; then
  DENO=(deno.exe)
elif command -v powershell.exe >/dev/null 2>&1 && powershell.exe -NoProfile -Command "deno --version" >/dev/null 2>&1; then
  DENO=(powershell.exe -NoProfile -Command deno)
elif command -v deno >/dev/null 2>&1 && deno --version >/dev/null 2>&1; then
  DENO=(deno)
else
  echo "Deno is required. Install Deno for Windows or run this script from a shell where 'deno' works." >&2
  exit 1
fi
