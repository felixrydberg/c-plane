    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConfigError::MissingIdentityDatabaseUrl => {
                write!(f, "IDENTITY_DATABASE_URL environment variable is required and cannot be empty")
            }
            ConfigError::MissingTenantDatabaseUrl => {
                write!(f, "TENANT_DATABASE_URL environment variable is required and cannot be empty")
            }
