...
async fn resolve_uncached(access_key: &str) -> Result<ResolvedS3AccessToken, AppError> {
    ...
    let rows = S3BucketPermissionRow::find_by_statement(Statement::from_sql_and_values(
        ...
        "SELECT grants.bucket_id AS foundation_bucket_id, COALESCE(storage.name, CASE WHEN managed.organization_id IS NOT NULL THEN 'registry' END, CASE WHEN clickhouse.id IS NOT NULL THEN 'clickhouse' END) AS bucket_name, COALESCE(storage_region.slug, registry_region.slug, provider.provider_region) AS region, foundation.s3_provider_id AS provider_id, grants.can_read, grants.can_write, foundation.status::text AS status FROM bucket_grant grants JOIN bucket foundation ON foundation.id=grants.bucket_id JOIN s3_providers provider ON provider.id=foundation.s3_provider_id LEFT JOIN storage_bucket storage ON storage.bucket_id=grants.bucket_id AND storage.organization_id IS NOT DISTINCT FROM grants.organization_id LEFT JOIN regions storage_region ON storage_region.id=storage.region_id LEFT JOIN managed_registry managed ON managed.bucket_id=grants.bucket_id AND managed.organization_id IS NOT DISTINCT FROM grants.organization_id LEFT JOIN regions registry_region ON registry_region.id=managed.region_id LEFT JOIN clickhouse_providers clickhouse ON clickhouse.bucket_id=grants.bucket_id AND clickhouse.storage_credential_id=grants.credential_id WHERE grants.credential_id=$1 AND provider.is_active=true AND (storage.id IS NOT NULL OR managed.organization_id IS NOT NULL OR clickhouse.id IS NOT NULL) ORDER BY bucket_name",
