...
async fn resolve_uncached(access_key: &str) -> Result<ResolvedS3AccessToken, AppError> {
    ...
    let rows = S3BucketPermissionRow::find_by_statement(Statement::from_sql_and_values(
        ...
        "SELECT grants.bucket_id AS foundation_bucket_id, COALESCE(storage.name, CASE WHEN managed.organization_id IS NOT NULL THEN 'registry' ELSE 'cp-' || replace(grants.bucket_id::text, '-', '') END) AS bucket_name, regions.slug AS region, regions.s3_provider_id AS provider_id, grants.can_read, grants.can_write, foundation.status::text AS status FROM bucket_grant grants JOIN bucket foundation ON foundation.id=grants.bucket_id JOIN regions ON regions.id=foundation.region_id JOIN s3_providers provider ON provider.id=regions.s3_provider_id LEFT JOIN storage_bucket storage ON storage.bucket_id=grants.bucket_id AND storage.organization_id IS NOT DISTINCT FROM grants.organization_id LEFT JOIN managed_registry managed ON managed.bucket_id=grants.bucket_id AND managed.organization_id IS NOT DISTINCT FROM grants.organization_id LEFT JOIN organization_region_backup_bucket backup ON backup.bucket_id=grants.bucket_id AND backup.organization_id=grants.organization_id WHERE grants.credential_id=$1 AND provider.is_active=true AND (storage.id IS NOT NULL OR managed.organization_id IS NOT NULL OR backup.id IS NOT NULL) ORDER BY bucket_name",
