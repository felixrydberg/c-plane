## Plan: RLS Rollout for ui + ui-studio

Enable PostgreSQL RLS end-to-end for Nuxt server APIs in ui and ui-studio using Drizzle-defined roles/policies, with request-scoped tenant context and phased migration from application-side organization filters to database-enforced isolation.
...
**Outcomes**
- ui and ui-studio server routes are RLS-safe.
...
1. Inventory all server endpoints in ui and ui-studio and map each to tables touched.
...
6. Add/adjust tenant policies with Drizzle pgPolicy + enableRLS only on tenant tables used by ui/ui-studio.
...
10. Add DB context helpers in ui and ui-studio server utils:
...
**Phase 4 - Route Migration (ui-studio)**
18. Apply same tenant context abstraction in ui-studio server.
...
25. Run migration + smoke tests in local/dev environments for ui and ui-studio.
26. Stage rollout:
...
- select+insert
- full CRUD
based on actual route behavior in ui and ui-studio.
...
- packages/ui-studio/server/utils/db.ts (or equivalent helper module)
- packages/ui-studio/server/api/**

**Success Criteria**
- Tenant data isolation in ui and ui-studio is guaranteed by RLS, not only app filters.
